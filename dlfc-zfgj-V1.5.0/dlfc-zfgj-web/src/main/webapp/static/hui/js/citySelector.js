...
/* 所有公司数据,可以按照格式自行添加（北京|beijing|bj），前16条为热门公司 */


/* 正则表达式 筛选中文公司名、拼音、首字母 */

Vcity.regEx = /^([~!/@#$%^&*()a-zA-Z0-9\u4E00-\u9FA5\uf900-\ufa2d\uFF00-\uFFFF]+)\|(\w+)\|(\w)\w*$/i;
Vcity.regExChiese = /([\u4E00-\u9FA5\uf900-\ufa2d]+)/;

/* *
 * 格式化公司数组为对象oCity，按照a-h,i-p,q-z,hot热门公司分组：
...
Vcity.CitySelector.prototype = {
    ...
    inputEvent:function(){
        ...
        Vcity._m.on(this.input,'focus',function(){
            that.input.select();
            if(that.input.value == '公司名') that.input.value = '';
        });
        Vcity._m.on(this.input,'blur',function(){
            if(that.input.value == '') that.input.value = '公司名';
        });
