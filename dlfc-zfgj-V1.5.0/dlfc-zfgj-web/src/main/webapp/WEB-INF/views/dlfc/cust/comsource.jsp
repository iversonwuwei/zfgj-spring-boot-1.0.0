...
<body>
	...
	<div class="content">
		...
		<form id="myForm" action="${ctx}/cust/source?sa=1" method="post">
		...
		<div class="list_head cl mt-20">
			<div class="r row cl col-3">
				<div class="formControls col-9 ">
					<input type="text" placeholder="客户姓名/状态/维护人" class="input-text"
						id="mycond">
				</div>
				<div class="formControls col-3">
					<a class="btn btn-primary" id="tijiao">搜索</a>
				</div>
			</div>

			<div class="list_tabs cl">
				<c:if test="${roleType != 'comp-role'&&roleType != 'assignment'}">
					<a id="1" href="${ctx}/cust/source?st=1"
						<c:if test="${sa == null||sa ==''}">class="active"</c:if>>我的客源</a>
					<c:if test="${enname == 'manager'}">
						<a id="0" href="${ctx}/cust/source?sa=1"
							<c:if test="${sa == 1}">class="active"</c:if>>部门客源</a>
					</c:if>
				</c:if>
			<%-- 	<c:if test="${roleType == 'comp-role'||roleType == 'assignment'}">
					<a id="0" href="${ctx}/cust/source?sa=1"
		...
		<div class="list_head cl">
			...
			<div class="house-list-tab cl">
				<a id="0" href="${ctx}/cust/source?sa=1"><span
					<c:if test="${st == null || st == '' }">class="current"</c:if>>全部</span></a>
				<a id="2" href="${ctx}/cust/source?st=1&sa=1"><span
					<c:if test="${ st == '1' }">class="current"</c:if>>未签</span></a> <a
					id="4" href="${ctx}/cust/source?st=4&sa=1"><span
					<c:if test="${st == 4}">class="current"</c:if>>签约</span></a>
			</div>
		</div>
		<div class="list_sort cl">
			<c:if test="${bs == null || bs ==  '' || bs == 'none'}">
				<a id="allsort" href="javascript:void(0);" title="无排序"
					onclick="doSort('butimesort', 'desc')">添加时间 <span>
						&uarr;</span></a>
			</c:if>
			<c:if test="${bs == 'desc'}">
				<a id="upsort" href="javascript:void(0);" title="降序"
					onclick="doSort('butimesort', 'asc')" class="active">添加时间 <span>
						&darr;</span></a>
			</c:if>
			<c:if test="${bs == 'asc'}">
				<a id="downsort" href="javascript:void(0);" title="升序"
					onclick="doSort('butimesort', 'none')" class="active">添加时间<span>
						&uarr;</span></a>
			</c:if>
		...
		<form name="custinfolist" action="${ctx}/cust/source">
			<input type="hidden" name="st" value="${st}"> <input
				type="hidden" name="sa" value="${sa}"> <input
				id="butimesort" type="hidden" name="bs" value="${bs}">
		...
		<form name="submitForm" action="${ctx}/cust/edit" method="post">
			<input id="custid" type="hidden" name="custid" value=""> <input
				id="require" type="hidden" name="require" value=""> <input
				id="repage" type="hidden" name="repage" value="1">
		...
		<div class="cl pd-5 bg-1 bk-gray mt-20">
			<shiro:hasPermission name="emp:cust:del">
				<span class="l"> <a href="javascript:;"
					onclick="datadel('${cust.id}');" class="btn btn-danger radius"><i
		...
		<table class="table table-border table-bordered  table-bg  mb-20">
			<tbody>
					<tr class="text-c">
						<td width="55"><input type="checkbox" value="${cust.id}"
							name="checkboxid"></td>
						...
						<td  width="300" id="requirementpanel"><c:forEach
								items="${cust.requirements}" var="requirement">

								<span class="mr-5" id="requirementsolo"> <a
									class="btn btn-primary size-MINI" style="margin-bottom: 0;">
							...
							</c:forEach> <input type="hidden" id="requirepanelid"> <input
							type="text" class="input-text size-MINI"
							style="width: 100px; display: none" id="requirepanel"
							onmouseout="doadd(this);"> <%-- <a class="btn btn-primary size-MINI" style="margin-bottom: 0;" onclick="addrequire('${cust.id}',this);">
									<i class="Hui-iconfont">&#xe600;</i>
								</a> --%></td>
						<td  width="80">
							<%-- <span class="select-box"> 
								<select class="select" disabled="disabled" size="1" name="status" onchange="changest('${cust.id}',this);">
						...
						</span> --%>${cust.statusName}</td>
						<td class="text-c"  width="80">
							<p class="house_name">${cust.personName}</p>
						</td>
									...
									<div class="info">
										<div class="responsive">
											...
											<c:forEach items="${cust.comments}" var="comment">
												<div class="row cl">
													<div class="col-2">
														<fmt:formatDate value="${comment.eventTime}"
	...
	<%-- 	<div id="addcust">
		<form id="addForm" name="addForm" class="form form_layer" method="post" action="${ctx}/cust/add">
			<br/>
			<div class="responsive">
				<div class="row cl">
			...
			<div class="row cl btn_group">
				<div class="col-9 col-offset-3">
					<a class="btn btn-primary radius" onclick="confirmAddDialog()">提交</a> 
	...
	<%-- <div id="editcust">
		<form id="editForm" name="editForm"  class="form form_layer" method="post" action="${ctx}/cust/edit">
			<br/>
			<div class="responsive">
				<div class="row cl">
	...
	<script type="text/javascript"
		src="${pageContext.request.contextPath}/static/hui/lib/laypage/1.2/laypage.js"></script>
	<script type="text/javascript"
		src="${pageContext.request.contextPath}/static/hui/lib/My97DatePicker/WdatePicker.js"></script>
	<script type="text/javascript"
		src="${pageContext.request.contextPath}/static/hui/lib/datatables/1.10.0/jquery.dataTables.min.js"></script>
	<script type="text/javascript"
	...
	<script type="text/javascript">
		$(function() {
			...
			$('#remarkForm').validationEngine('attach', {
				binded : false,
				showOneMessage : true,
				maxErrorsPerField : 1,
				promptPosition : 'centerRight',
		...
		function doDelete(id) {
			layer.confirm('如删除此客户信息则经纪人客源列表中的此记录也将删除，是否继续？', {
				btn : [ '确认', '返回' ]
			//按钮
			}, function() {
				var tar = "${ctx}/cust/delc";
