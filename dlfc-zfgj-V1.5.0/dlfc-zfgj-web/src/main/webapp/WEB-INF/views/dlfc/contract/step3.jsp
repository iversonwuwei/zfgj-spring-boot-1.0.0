...
<body>
	...
	<form action="${ctx}/con/step3" method="get" id="getForm">
		<input type="hidden" id="cid" name="cid" value="${con.id}">
	</form>
	<div class="pd-20">
		<form action="${ctx}/con/step3" method="post" class="form form-horizontal" id="step3Form">
			...
			<div class="responsive">
				<div class="row cl">
					...
					<c:when test="${con.stages == '01' }">
						<div class="radio-box">
							<form:radiobutton path="con.settlementCycle" id="settlementCycle4" onclick="setPayTime(this)" value="4" class="ml20" checked="true"/><label for="settlementCycle4">年付 </label>
						</div>
					</c:when>
					<c:otherwise>
						<div class="radio-box">
			...
			<%-- <div class="responsive">
				<div class="row cl" id="payTimeDiv">
					<c:choose>
						<c:when test="${con.settlementCycle != null && con.settlementCycle != 1}">
							<label class="form-label col-3"><span class="c-red">*</span>支付时间：</label>
							<p class="form_text">每周期第</p>
							<p class="form_text">
								<span class="select-box"><form:select
											path="con.payTimeMonth" id="payTimeMonth" onchange="countPayTime()" class="select">
											<c:choose>
												<c:when test="${con.settlementCycle == 2 }">
													<c:forEach var="i" begin="1" end="3" varStatus="status">
														<form:option value="${i }" />
													</c:forEach>
												</c:when>
												<c:when test="${con.settlementCycle == 3 }">
													<c:forEach var="i" begin="1" end="6" varStatus="status">
														<form:option value="${i }" />
													</c:forEach>
												</c:when>
												<c:when test="${con.settlementCycle == 4 }">
													<c:forEach var="i" begin="1" end="12" varStatus="status">
														<form:option value="${i }" />
													</c:forEach>
												</c:when>
											</c:choose>
							...
							<p class="form_text">
									<span class="select-box"> <form:select
											path="con.payTimeDay" id="payTimeDay" onchange="countPayTime()" class="select">
			...
			<div class="responsive">
				<div class="row cl">
					...
					<div class="formControls col-1">
						<form:input path="con.depositAmt" id="depositAmt" class="validate[required,custom[number],min[0]] input-text" placeholder="保证金" data-prompt-target="depositAmtError" data-prompt-position="inline"/>
					</div>
					<p class="form_text">元</p>
				...
				<%-- <div class="row cl">
					<label class="form-label col-3">押金监管：</label>
					<div class="formControls col-2">
						<div class="skin-minimal">
							<div class="radio-box">
								<form:checkbox path="con.paymentMethod" id="paymentMethod" value="1"/><label for="paymentMethod">申请押金监管服务</label>&nbsp;&nbsp;<a onclick="depositInfo()" class="c-blue">《押金监管协议书》</a>
							</div>
						</div>
					</div>
				</div> --%>
			...
			<div class="row cl btn_group">
				<div class="col-9 col-offset-3">
					<a class="btn btn-default radius" href="${ctx}/con/step2?cid=${con.id}">上一步</a> <a
