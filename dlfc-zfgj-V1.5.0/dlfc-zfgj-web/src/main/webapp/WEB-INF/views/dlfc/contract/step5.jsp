...
<body>
	...
	<div class="pd-20">
		<form action="${ctx}/con/renewal" method="post"
			...
			<c:choose>
				...
				<c:otherwise>
					<c:if test="${con.status == 1}">
						<div class="row cl text-r">
							<a class="btn-link" href="${ctx}/con/step1?cid=${con.id}">修改</a>
						</div>
					</c:if>
				</c:otherwise>
			</c:choose>
			<div class="responsive mb-30">
				<div
					...
					<p class="contract_til col-1-1">
						${con.houseAddr}<span class="label label-primary ml-10">${con.rentalModeName}</span><c:if test="${con.hasInsurance == 1}"> <a onclick="insuranceInfo()" class="c-blue">即日起，在大连市房屋租赁公共服务平台签约一年以上（含一年）房屋租赁合同的房主，将获得一份保额价值为13万的租房无忧险。 </a></c:if>
					</p>
							...
							<div class="row cl text-c">
								<c:choose>
									<c:when test="${con.status == 13 || con.status == 10}">
										<c:if test="${con.stages == '01' }">
										<a class="btn btn-primary radius" onclick="openUploadDialog('1200px', '700px')">上传合同</a>
										</c:if>
										<c:if test="${con.stages != '01' }">
										<a class="btn btn-primary radius" onclick="openUploadDialog('1200px', '400px')">上传合同</a>
										</c:if>
									</c:when>
									<c:when
										...
										<shiro:hasPermission name="emp:cont:add">
										<c:if test="${con.stages == '01' }">
										<a class="btn btn-primary radius" onclick="openUploadDialog('1200px', '650px')">查看上传</a>
										</c:if>
										<c:if test="${con.stages != '01' }">
										<a class="btn btn-primary radius" onclick="openUploadDialog('1200px', '350px')">查看上传</a>
										</c:if>
										</shiro:hasPermission>
									</c:when>
									<c:otherwise>
			...
			<c:forEach var="owner" items="${con.ownerList}"
				varStatus="status">
					...
					<div class="panel-body">
						...
						<div class="responsive">
							<div class="row cl">
								...
								<div class="form_text col-2">${owner.sysPerson.mobile}</div>
							</div>
						</div>
					</div>
				</div>
				...
				<c:when test="${con.stages == '01' }">
					<div class="responsive">
						<div class="row cl">
							...
							<div class="form_text col-4">${con.settlementCycleName} <span class="c-orange">租金分期：</span><a href="#" class="btn-link ml-10" onclick="lessorclause()">《房租一次性支付分期还款服务协议（出租方）》</a><a href="#" class="btn-link ml-10" onclick="lesseeclause()">《房租一次性支付分期还款服务协议（承租方）》</a></div>
							<label class="form-label col-3">结算时间：</label>
							<div class="form_list col_1_1 fn_clear">
								<p class="form_name">&nbsp;</p>
								<div class="contract_pay_tips"
									style="width: 600px; display: inline-block; *display: inline; *zoom: 1;">
									<ul id="payTimeUl">
									</ul>
								</div>
							</div>
	...
	<div id="uploadDiv" style="display: none">
		...
		<form action="${ctx}/con/uploCon" method="post"
			class="form form-horizontal" id="uploadContract">
			<form:hidden path="con.id" />
			<form:hidden path="con.version" />
			<input type="hidden" name="infoAttList[0].filePath"
				id="oneSideFilePath" value="${con.infoAttList[0].filePath}" /> 
			...
			<input type="hidden" name="infoAttList[0].fileName"
				id="oneSideFileName" value="${con.infoAttList[0].fileName}" /> 
			<input type="hidden" name="infoAttList[1].filePath" id="otherSideFilePath"
				value="${con.infoAttList[1].filePath}" /> 
			<input type="hidden" name="infoAttList[1].fileRealName" id="otherSideFileRealName"
				value="${con.infoAttList[1].fileRealName}" /> 
			<input type="hidden" name="infoAttList[1].fileName" id="otherSideFileName"
				value="${con.infoAttList[1].fileName}" />
			<input type="hidden" name="infoAttList[2].filePath" id="securityBookFilePath"
				value="${con.infoAttList[2].filePath}" /> 
			<input type="hidden" name="infoAttList[2].fileRealName" id="securityBookFileRealName"
				value="${con.infoAttList[2].fileRealName}" /> 
			<input type="hidden" name="infoAttList[2].fileName" id="securityBookFileName"
				value="${con.infoAttList[2].fileName}" />
			<input type="hidden" name="infoAttList[3].filePath" id="lessorFilePath"
				value="${con.infoAttList[3].filePath}" /> 
			<input type="hidden" name="infoAttList[3].fileRealName" id="lessorFileRealName"
				value="${con.infoAttList[3].fileRealName}" /> 
			<input type="hidden" name="infoAttList[3].fileName" id="lessorFileName"
				value="${con.infoAttList[3].fileName}" />
			<input type="hidden" name="infoAttList[4].filePath" id="lesseeFilePath"
				value="${con.infoAttList[4].filePath}" /> 
			<input type="hidden" name="infoAttList[4].fileRealName" id="lesseeFileRealName"
				value="${con.infoAttList[4].fileRealName}" /> 
			<input type="hidden" name="infoAttList[4].fileName" id="lesseeFileName"
				value="${con.infoAttList[4].fileName}" />
			
			<c:choose>
			<c:when test="${con.stages == '01' }">
			<div class="row cl">
				<div class="form_text col-4 text-c">上传合同正面：</div>
				<div class="form_text col-4 text-c">上传合同背面：</div>
			</div>
			<div class="row cl ">
				<div class=" formControls text-c col-4">
			...
			<div class="row cl">
				<div class="form_text col-6 text-c">上传出租方分期服务协议：</div>
				<div class="form_text col-6 text-c">上传承租方分期服务协议：</div>
			</div>
			<div class="row cl ">
			<!-- 两份协议 -->
				<div class=" formControls text-c col-6">
					<div id="uploader4" class="wu-example">
						<!--用来存放文件信息-->
						<div id="fileList4" class="fileList">
							<c:if
								test="${!empty con.infoAttList[3].filePath && !empty con.infoAttList[3].fileRealName && !empty con.infoAttList[3].fileName}">
								<div id="WU_FILE_0" class="file-item thumbnail">
									<a
										href="${pageContext.request.scheme}://${pageContext.request.serverName}:${pageContext.request.serverPort}/img/${con.infoAttList[3].filePath}"
										class="fancybox"><img
										src="${pageContext.request.scheme}://${pageContext.request.serverName}:${pageContext.request.serverPort}/img/${con.infoAttList[3].filePath}?key=${accessKey}" /></a>
									<div class="info" title=" ${con.infoAttList[3].fileRealName} ">${con.infoAttList[3].fileRealName}</div>
								</div>
							</c:if>
						</div>
						<c:if test="${con.status == 13 || con.status == 10}">
							<div class="btns">
								<div id="filePicker4" style="float: none;">选择图片</div>
							</div>
						</c:if>
					</div>
				</div>
				<div class=" formControls text-c col-6">
					<div id="uploader5" class="wu-example">
						<!--用来存放文件信息-->
						<div id="fileList5" class="fileList">
							<c:if
								test="${!empty con.infoAttList[4].filePath && !empty con.infoAttList[4].fileRealName && !empty con.infoAttList[4].fileName}">
								<div id="WU_FILE_0" class="file-item thumbnail">
									<a
										href="${pageContext.request.scheme}://${pageContext.request.serverName}:${pageContext.request.serverPort}/img/${con.infoAttList[4].filePath}"
										class="fancybox"><img
										src="${pageContext.request.scheme}://${pageContext.request.serverName}:${pageContext.request.serverPort}/img/${con.infoAttList[4].filePath}?key=${accessKey}" /></a>
									<div class="info" title=" ${con.infoAttList[4].fileRealName} ">${con.infoAttList[4].fileRealName}</div>
								</div>
							</c:if>
						</div>
						<c:if test="${con.status == 13 || con.status == 10}">
							<div class="btns">
								<div id="filePicker5" style="float: none;">选择图片</div>
							</div>
						</c:if>
					</div>
				</div>
	...
	<script type="text/javascript">
		...
		$(function() {
			...
			uploader = WebUploader.create({
				...
				server : '${ctx}/con/upload?cid=${con.id}&fileType=1',//上传的URL
				accept : {
					title : '请上传图片格式',
			...
			uploader2 = WebUploader.create({
				...
				swf : '${ctxStatic}${hui}lib/webuploader/0.1.5/Uploader.swf',
				server : '${ctx}/con/upload?cid=${con.id}&fileType=2',//上传的URL
				accept : {
					title : '请上传图片格式',
					extensions : 'jpg,jpeg,bmp,png'
				},
			...
			uploader2.on('uploadSuccess', function(file) {
				...
				if (arguments[1].success == 0) {
					...
					$("#securityBookFilePath").val(arguments[1].data.filePath);
					$("#securityBookFileName").val(arguments[1].data.fileName);
					$("#securityBookFileRealName").val(arguments[1].data.fileRealName);
				}
			});
			...
			uploader3 = WebUploader.create({
				...
				swf : '${ctxStatic}${hui}lib/webuploader/0.1.5/Uploader.swf',
				server : '${ctx}/con/upload?cid=${con.id}&fileType=22',//上传的URL
				accept : {
					title : '请上传图片格式',
					extensions : 'jpg,jpeg,bmp,png'
				},
			...
			uploader4 = WebUploader.create({
				...
				swf : '${ctxStatic}${hui}lib/webuploader/0.1.5/Uploader.swf',
				server : '${ctx}/con/upload?cid=${con.id}&fileType=4',//上传的URL
				accept : {
					title : '请上传图片格式',
					extensions : 'jpg,jpeg,bmp,png'
				},
			...
			uploader4.on('uploadSuccess', function(file) {
				...
				if (arguments[1].success == 0) {
					...
					$("#lessorFilePath").val(arguments[1].data.filePath);
					$("#lessorFileName").val(arguments[1].data.fileName);
					$("#lessorFileRealName").val(arguments[1].data.fileRealName);
				}
			});
			...
			uploader5 = WebUploader.create({
				...
				swf : '${ctxStatic}${hui}lib/webuploader/0.1.5/Uploader.swf',
				server : '${ctx}/con/upload?cid=${con.id}&fileType=5',//上传的URL
				accept : {
					title : '请上传图片格式',
					extensions : 'jpg,jpeg,bmp,png'
				},
			...
			uploader5.on('uploadSuccess', function(file) {
				...
				if (arguments[1].success == 0) {
					...
					$("#lesseeFilePath").val(arguments[1].data.filePath);
					$("#lesseeFileName").val(arguments[1].data.fileName);
					$("#lesseeFileRealName").val(arguments[1].data.fileRealName);
				}
			});
			initUploader(uploader4, 150, 150, "#fileList4");
			initUploader(uploader5, 150, 150, "#fileList5");
			</c:if>
			initUploader(uploader, 150, 150, "#fileList");
			initUploader(uploader3, 150, 150, "#fileList3");
			initUploader(uploader2, 150, 150, "#fileList2");
				
			<c:choose>
				<c:when test="${isRenewal}">
					$("#renewalForm").attr("action", "${ctx}/con/renewal");
				</c:when>
				<c:otherwise>
					$("#renewalForm").attr("action", "${ctx}/con/step5");
				</c:otherwise>
			</c:choose>
		...
		function confirmDialog() {
			layer.confirm("点击此按钮，合同内容将永远不能修改，确定要下载合同吗？", function(){
				var startTime = new Date('${con.startTime}');
				var endTime = new Date('${con.endTime}');
					...
					success : function(data) {
						if (1 == data.success) {
							...
							layer.alert(data.message,function index(){
								window.location.href = "${ctx}/con/clist";
							});
						}
					}
				});
		...
		function submitUploadForm() {
			<c:if test="${con.stages == '00' }">
			if ($("#oneSideFilePath").val() == '' || $("#oneSideFileName").val() == '' || $("#oneSideFileRealName").val() == '') {
				layer.alert("请上传合同正面", {icon : 5});
			}else if ($("#securityBookFilePath").val() == '' || $("#securityBookFileName").val() == '' || $("#securityBookFileRealName").val() == '') {
			...
			if ($("#oneSideFilePath").val() == '' || $("#oneSideFileName").val() == '' || $("#oneSideFileRealName").val() == '') {
				...
				layer.alert("请上传合同背面", {icon : 5});
			} else if ($("#lessorFilePath").val() == '' || $("#lessorFileName").val() == '' || $("#lessorFileRealName").val() == '') {
				layer.alert("上传出租方分期服务协议", {icon : 5});
			} else if ($("#lesseeFilePath").val() == '' || $("#lesseeFileName").val() == '' || $("#lesseeFileRealName").val() == '') {
				layer.alert("上传承租方分期服务协议", {icon : 5});
			} else {
				$("#uploadContract").submit();
			}
			</c:if>
