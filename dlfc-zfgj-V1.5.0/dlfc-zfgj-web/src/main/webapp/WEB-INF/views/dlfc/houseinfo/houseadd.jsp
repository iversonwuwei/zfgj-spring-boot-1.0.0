...
<body>
	<nav class="breadcrumb">
		...
		<form action="javascript:void(0)" method="post" class="form form-horizontal" id="houseadd" enctype="multipart/form-data">
			...
			<input type="hidden" id="file_real_name" name="file_real_name" value="${file_real_name}">
			<input type="hidden" id="per_id" name="per_id" value="${per_id}">
			<input type="hidden" id="ent_id" name="ent_id" value="${ent_id}">
			
			<legend>房源信息</legend>
			<div class="row cl">
				<label class="form-label col-3"><span class="c-red">*</span>详细地址：</label>
				<div class="formControls col-5">
					<form:input type="text" class="validate[required,maxSize[100],minSize[1],funcCall[checkAddr]] input-text" path="houseinfo.houseAddr" id="houseAddr"  maxlength="100" placeholder="如黄河路"/>
				</div>
			...
			<div class="responsive cl mb-10">
				<div class="cl row form_owner ">
					...
					<div class="formControls mb-10 col-6">
						<form:input type="text" class="validate[required,minSize[2],maxSize[15],funcCall[checkName]] input-text" path="houseinfo.houseOwnerName" id="houseOwnerName"  maxlength="10" placeholder="房主姓名"/>
					</div>
					<div class="col-3"></div>
					<div class="cl"></div>
					<label class="form-label col-3 removeSpan"><span class="c-red">*</span>房主证件：</label>
					<div class="formControls mb-10 col-3">
						<span class="select-box">
						<form:select path="houseinfo.houseOwnerIdType" id="houseOwnerIdType" class="select" size="1">
				...
				<c:forEach items="${houseinfo.owners}" var="coOwner">
					<div class="co_box cl row form_owner ">	
						...
						<div class="formControls mb-10 col-6">
							<input type="text" placeholder="共有人姓名" class="validate[required,minSize[2],maxSize[15],funcCall[checkName]] input-text" maxlength="10" value="${coOwner.name}" name="coname" id="coname">
						</div>
						...
						<div class="queueList" id="queueList">
							<ul class="filelist">
								...
								<li class="state-complete" onmouseenter="bigImg(this)">
									<p class="title"></p>
									<p class="imgWrap">
										<img src="${pageContext.request.scheme}://${pageContext.request.serverName}:${pageContext.request.serverPort}/temp/${att}" width="110" height="110">
									</p>
									<p class="moreInfo">
										<input type="hidden" name="pics" value="${att}">
									</p>
									<p class="progress">
			...
			<div class="row cl btn_group">
				<div class="col-9 col-offset-3">
					<a class="btn btn-primary radius" id="saveInfo">添加房源</a> 
				</div>
			</div>
	...
	<div id="hou_co_owner" style="display: none;">
		<div class="co_box cl row form_owner ">
			<label class="form-label col-3"><span class="c-red">*</span>共有人姓名：</label>
			<div class="formControls mb-10 col-6">
	...
	<script type="text/javascript">
		...
		idImgUploader=mulImagesInit('${ctx}','',file_size,swfPath);
		...
		$('#saveInfo').on('click',function() {
			var success = $("#houseadd").validationEngine("validate");
			if(!success){
				return;
			}
			...
			$("input[name=coidtype]").each(function(){
				alert(this.name);
			});
			var propertyIdType=$("#propertyIdType").val();
			// 房主信息是空时
			...
			if(!checkId(propertyIdNo)){
				layer.confirm(
					...
					//按钮
						...
						window.location.href="${ctx}/house/list" ;
					}, function() {
						//layer.msg('返回成功');
					});
			}else{
				$('#saveInfo').unbind("click");
				var form = document.forms['houseadd'];
				form.action="${ctx}/house/add"
				form.submit();
			}
		});
