...
<body>
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 合同管理<span class="c-gray en">&gt;</span> 合同列表<a class="btn btn-success radius r mr-20" style="line-height:1.6em;margin-top:3px" href="javascript:location.replace(location.href);" title="刷新" ><i class="Hui-iconfont">&#xe68f;</i></a></nav>
<div class="content">
  <div class="mt-20">
	<form id="conSearch" action="${ctx}/con/clist" method="get">
		<div class="r row cl col-3">
			<div class="formControls col-9 ">
				<input type="text" class="input-text" placeholder="合同编号/签约经纪人/出租方/承租方" name="searchInfo" value="${searchInfo}">
			</div>
    ...
    <a onclick="doSort('enname', '')">个人合同</a>
    	</c:when>
    	<c:otherwise>
    ...
    <form name="contractlist" action="${ctx}/con/clist" id="contractlist">			
			...
			<input type="hidden" id="enname" name="enname" value="${root}" />
    ...
    <a href="${ctx}/con/clist?st=5&enname=${root}" ><span <c:if test="${st == 5}">class="current"</c:if>>生效中 </span></a><a href="${ctx}/con/clist?st=&enname=${root}"><span <c:if test="${st == null || st == '' }">class="current"</c:if>>全部</span></a>
    	</div>
    </div>
    ...
    <table class="table table-border table-bordered  table-bg">
      <thead>
        ...
        <tr class="text-c">
          ...
          <td class="text-c" width="140">${contractlist.agtEmpInfo.sysPerson.name}</td>
          ...
          <td width="100"><p class="c-red">合同终止</p>
            <a href="${ctx}/con/step5?cid=${contractlist.id}">合同详情</a></td>
          <td class="text-c" width="140">${contractlist.agtEmpInfo.sysPerson.name}  
          </td>
          <td class="text-c" width="140"> <a class="btn btn-primary radius"  href="${ctx}/con/step5?cid=${contractlist.id}">查看详情</a>  
          </td>
          	</c:when>
          ...
          <td class="text-c" width="140">  
			${contractlist.agtEmpInfo.sysPerson.name}
          </td>
          <td class="text-c" width="140"> <a class="btn btn-primary radius"  href="${ctx}/con/step5?cid=${contractlist.id}">查看详情</a>  
          </td>
          	</c:otherwise>
