...
<body>
	...
	<aside class="Hui-aside">
		...
		<input id="menuList" type="hidden" value="${fns:getMenuList()}" />
					...
					<c:forEach items="${fns:getMenuList3(menu)}" var="cmenu"
						...
						<c:if test="${cmenu.isShow eq '1'}">
							<dl id="menu-article">
								...
								<dd>
									<ul>
										<c:forEach items="${fns:getMenuList3(cmenu)}" var="dmenu"
											varStatus="idxStatus2">
											<!-- 初始化首页 -->
											<c:if test="${not empty dmenu.href and hasFirst=='true'}">
												<c:set var="firstMenu"
													value="${fn:indexOf(dmenu.href, '://') eq -1 ? ctx : ''}${dmenu.href}"></c:set>
												<c:set var="firstMenuName" value="${dmenu.name }"></c:set>
												<c:set var="hasFirst" value="false"></c:set>
											</c:if>
											<c:if test="${dmenu.isShow eq '1'}">
												<li><c:if test="${empty dmenu.href}">
														...
														<a
															_href="${fn:indexOf(dmenu.href, '://') eq -1 ? ctx : ''}${dmenu.href}"
															href="javascript:void(0)"><i
															class="boot_icons ${dmenu.icon }"></i> ${dmenu.name }</a>
													</c:if></li>
											</c:if>
	...
	<section class="Hui-article-box">
		<div id="Hui-tabNav" class="Hui-tabNav">
			<div class="Hui-tabNav-wp">
				<ul id="min_title_list" class="acrossTab cl">
					<li class="active"><span title="${firstMenuName }"
		...
		<div id="iframe_box" class="Hui-article">
			<div class="show_iframe">
				...
				<iframe scrolling="yes" frameborder="0" src="${firstMenu }"></iframe>
			</div>
		</div>
	</section>
