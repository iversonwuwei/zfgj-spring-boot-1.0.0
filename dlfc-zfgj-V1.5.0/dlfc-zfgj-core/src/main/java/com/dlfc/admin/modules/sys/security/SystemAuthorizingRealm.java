...
public class SystemAuthorizingRealm extends AuthorizingRealm {
	...
	protected AuthenticationInfo doGetAuthenticationInfo(
		...
		// 本公司管理员登录验证
		if (FormAuthenticationFilter.MGR_LOGIN_TYPE
				.equals(token.getLoginType())) {
			// 校验用户名密码
			CacheUtils.remove("userCache", "ln" + token.getUsername());
			User user = getSystemService().getUserByLoginName(
					token.getUsername());

			if (user != null) {
				if (!(""+UserIdentityEnum.MEDI_USER_ENUM.getValue()).equals(user.getUserType())
						&& !(""+UserIdentityEnum.BROKER_USER_ENUM.getValue()).equals(user.getUserType())) {
					return null;
				...
				return new SimpleAuthenticationInfo(new Principal(user,
						token.isMobileLogin(), token.getLoginType()), user
						.getPassword().substring(16),
			...
			// 校验用户名密码
			User user = usrUserService.getUserByLoginName(token.getUsername());
			if (user != null) {
				if (Global.NO.equals(user.getLoginFlag())) {
					throw new AuthenticationException("msg:该帐号已禁止登录.");
				}
