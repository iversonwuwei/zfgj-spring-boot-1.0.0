...
import com.dlfc.zfgj.service.mobile.SMSMService;
 ...
 */
@Transactional
@Service("smsMServiceImpl")
public class SMSMServiceImpl implements SMSMService {
    /**
     * 手机短信验证码Mapper
     */
    @Autowired
    ...
    public String sendCapcha(String mobile, String templateNo, String domain, HttpServletRequest request) {
        if (StringUtils.isBlank(mobile) || StringUtils.isBlank(templateNo) || StringUtils.isBlank(domain)) {
            ...
            String capcha = SMSFacet.sendCapcha(mobile, request.getSession().getId(), "127.0.0.1", templateNo, domain);
            return capcha;
        }
    }
