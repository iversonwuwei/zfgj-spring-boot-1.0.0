...
import com.dlfc.zfgj.dto.web.ContractStep5InitDTO;
...
@Component
public class ContractStep5InitConvertor extends AbstractConvertor<ConContract, ContractStep5InitDTO> {
    // 房主承担
    ...
    @Autowired
    private SystemAttachmentConvertor systemAttachmentConvertor;
    @Autowired
    ...
    @Override
    public ConContract toModel(ContractStep5InitDTO dto) {
        return null;
    }

    @Override
    public ContractStep5InitDTO toDTO(ConContract model, boolean forListView) {
        ContractStep5InitDTO dto = new ContractStep5InitDTO();
        if (null != model) {
            ...
            if (null != model.getLessorIdType()) {
                dto.setLessorIdType(
                        sysCodeServiceImpl.getNameByType(model.getLessorIdType().toString(), PER_ID_TYPE));
            }
            ...
            if (null != model.getLesseeIdType()) {
                dto.setTenantryIdType(
                        sysCodeServiceImpl.getNameByType(model.getLesseeIdType().toString(), PER_ID_TYPE));
            }
...
//            dto.setHouseOwnerDTOList(houseOwnerConvertor.toListDTO(sysPersonList));
            ...
            if (null != model.getHouseOwnerIdType()) {
                houseOwnerDTO.setIdType(
                        sysCodeServiceImpl.getNameByType(model.getHouseOwnerIdType().toString(), PER_ID_TYPE));
            }
            ...
            if (null != conHouseUserList) {
                dto.setHouUserList(houseUserConvertor.toListDTO(conHouseUserList));
            }
            // 租赁期限
            ...
            String lessorIds = getStrFromListIfExist(
                    sysCodeServiceImpl.getCodeByType(LESSOR_ORIGI_LICENSE), model.getLessorIds());
            ...
            String lesseeIds = getStrFromListIfExist(
                    sysCodeServiceImpl.getCodeByType(LESSEE_ORIGI_LICENSE), model.getLesseeIds());
            ...
            if (null != houItemsList) {
                dto.setDeliverItems(houseItemsConvertor.toListDTO(houItemsList));
            }
            // 其他相关费用
            List<ConOtherCosts> conOtherCostsList = conOtherCostsService.getConOtherCostsByCid(model.getId());
            if (null != conOtherCostsList) {
                dto.setOtherRelatedItems(contractOtherCostConvertor.toListDTO(conOtherCostsList));
            }
            //随住子女
            List<ConHouseUserChildren> conHouseUserChildrens = conHouseUserChildrenService.getByCid(model.getId());
            if (null != conHouseUserChildrens){
                dto.setChildrenMDTOS(conHouseUserChildrenConvertor.toListDTO(conHouseUserChildrens));
            }
            // 附件列表
            List<SysInfoAtt> sysInfoAttList = sysInfoAttServiceImpl.findList(model.getId(),
            ...
            if (null != sysInfoAttList) {
                dto.setSystemAttachmentDTOList(systemAttachmentConvertor.toListDTO(sysInfoAttList));
            }
            // 程序读取加密（防盗链配置的图片方式）
            stringBuffer = new StringBuffer();
            ...
            AgtEmpInfo agtEmpInfo = agtEmpInfoService.getAgtEmpInfoById(model.getEid());
            if (null != agtEmpInfo) {
            ...
            if (null != startTime
                ...
                dto.setPaymentExplanation(
                        contractStepService.getPaymentExplanation(
                                startTime,
                                endTime,
                                settlementCycle,
                                monthlyRent,
